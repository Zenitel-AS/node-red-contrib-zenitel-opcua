            ...
            switch (payload.opcuaCommand) {
                ...
                case "installDiscreteAlarm":
                    ...
                    if (nodeId) {
                        ...
                        var alarmText = namespace.addVariable({
                            nodeId: nodeStr + "-" + "AlarmText",
                            browseName: nodeStr.substring(7) + "-" + "AlarmText",
                            displayName: nodeStr.substring(7) + "-" + "AlarmText",
                            propertyOf: nodeId,
                            dataType: "String",
                            minimumSamplingInterval: 500, // NOTE alarm is event based DO NOT USE THIS! ???
                            eventSourceOf: "i=2253", // Use server as default event source
                        ...
                        alarmText.setValueFromSource({dataType: "String", value: "No Event"});


						RED.log.info("EM: nodeId = " +  nodeStr + "-" + "DiscreteAlarm");
						RED.log.info("EM: browseName = " +  nodeStr.substring(7) + "-" + "DiscreteAlarm");
						RED.log.info("EM: displayName = " +  nodeStr.substring(7) + "-" + "DiscreteAlarm");

                        var discreteAlarm = addressSpace.findEventType("DiscreteAlarmType");

					
                        var alarmMess = namespace.instantiateDiscreteAlarm(discreteAlarm,
                            {
                                nodeId: nodeStr + "-" + "DiscreteAlarm",
                                browseName: nodeStr.substring(7) + "-" + "DiscreteAlarm",
                                displayName: nodeStr.substring(7) + "-" + "DiscreteAlarm",
                                organizedBy: nodeId,
                                conditionSource: alarmText,
                                severity: priority,
                                conditionName: "Node-Red OPC UA Event",
                                browseName: "DiscreteAlarmInstance",
                                inputNode: alarmText,   // the variable that will be monitored for change, generate below
                                optionals: [ "Acknowledge", "ConfirmedState", "Confirm" ], // confirm state and confirm Method
                            }
                        );
												
                        alarmMess.setEnabledState(true);
                        // alarmMess.setSeverity(priority);
                        // Do not use default
						
                        try
						{ 
						 
                            alarmText.on("value_changed", function (newDataValue)
							{						
								var txt = newDataValue.value.value;
								RED.log.info("EM: Alarm Text Changed: " + "\"" + newDataValue.value.value + "\"");
								
                                if (txt == "")
								{
                                    //alarm.deactivateAlarm();
									RED.log.info("EM: Deactivate Alarm.");
                                }
								else
								{
									//alarmMess.activeState.setValue(true);
									//alarmMess.ackedState.setValue(false);
									
									alarmMess.activeState.setValue(true);
									alarmMess.ackedState.setValue(true);

									alarmMess.raiseNewCondition
									({
										severity: priority,
										message: txt,								
										retain: true
